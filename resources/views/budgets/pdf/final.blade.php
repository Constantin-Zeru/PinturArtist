...
<head>
    <meta charset="UTF-8">
    <title>Presupuesto final #{{ $budget->id }}</title>
    <style>
        @page {
...
@php
    ...
    $approvedChanges = $changes->where('status', 'aprobado');
    $pendingChanges = $changes->where('status', 'pendiente');
    $rejectedChanges = $changes->where('status', 'rechazado');
...
<div class="header">
    <table class="header-table">
        <tr>
            <td>
                <div class="brand">Presupuesto final</div>
                <div class="subtitle">Base + modificaciones aplicadas</div>
            </td>
            <td class="meta">
...
@if($showInternal)
    <p><strong>Margen interno:</strong> {{ number_format((float) $budget->profit_margin, 2, ',', '.') }} %</p>
@endif
...
<div class="section">
    ...
    <table class="totals">
        <tr>
            ...
            <td class="total-value">{{ number_format($baseTotal, 2, ',', '.') }} €</td>
        </tr>
        <tr class="success">
        ...
        <tr class="highlight">
            ...
            <td class="total-value">{{ number_format($finalTotal, 2, ',', '.') }} €</td>
        </tr>
    </table>
</div>
        ...
        <tbody>
            @forelse($changes as $change)
                <tr>
                    ...
                    <td class="{{ (float) $change->amount < 0 ? 'danger' : '' }}">
